...
현재 기준 백엔드는 **NestJS API + Nest worker**입니다. Python FastAPI/Jinja와 Electron 관리자는 제거했고, Adrock 사내 관리자/공개 API의 실행 기준은 `apps/api-nest`입니다.
...
SQLite DB는 `adrock-db` 볼륨의 `/data/admin.db`에 저장됩니다.
...
Next 사내 관리자 프록시는 `.env.local`의 `ADMIN_API_TOKEN` 값을 API로 전달합니다.
