...
- **학원 소개·비교 글의 정본은 T16(`local_axis`)이다. T01 계열은 폐기됐다** — `constants.ts`의 `DEPRECATED_BUILTIN_TEMPLATE_IDS`에 있고, 노출 목록에 넣어도 서버(`settings/builtin-visibility`)가 걸러내 다시 켜지지 않는다. 코드(`t01-*.ts`)는 과거 발행 글·슬롯이 참조하므로 남겨 둔다. **학원 글 규칙을 바꿀 때 `t01-legacy-plus.ts`·`t01-data-gated.ts`·`t01-legacy-plus-postedit.ts`를 고치면 아무 효과가 없다** — 지금 도는 글은 전부 T16이다. 고칠 곳은 `t16-axis-comparison.ts`(T16 전용 계약·문체)와 `archetypes.ts`의 `local_axis`(구조)다. **함정: `academy-review-evidence.ts`는 이름이 중립적이지만 `selectedStudentReviewForAcademy`·`selectEligibleReviewsByAcademy`는 T01 경로에서만 불린다.** 리뷰 규칙처럼 두 계열이 공유해야 하는 판정은 연결 지점인 `academy-link.service.ts`에 두면 양쪽에 닿는다(실제로 리뷰 인용 필터가 T01 경로에만 걸려 있어 T16 글이 새던 적이 있다).
