...
- `npm run verify:doc-sync` — **문서**가 열거한 코드 사실(관리자 라우트·엔드포인트 수·도메인 관리 탭)이 실제와 맞는지 검사. 수동이다(pre-commit 아님). 화면·컨트롤러를 늘리거나 지웠으면 돌려라.
...
- **LLM 텍스트 생성은 API 키가 아니라 CLI 서브프로세스 + OAuth에 의존한다.** `runLlm()`(**`llm-runner.ts`** — 한때 `worker.service.ts`에 있었고 관리자 축 값 제안이 같은 코드를 쓰게 되면서 공용 추출됐다. 워커·`admin.controller`·`src/scripts/*` 스모크가 함께 부른다)이 `codex exec` 또는 `claude --print` 바이너리를 서브프로세스로 spawn해 스트림 JSON을 파싱한다. 기본 프로바이더는 `codex`. Claude 경로에서는 OAuth 강제를 위해 서브프로세스 env의 `ANTHROPIC_API_KEY`/`ANTHROPIC_AUTH_TOKEN`을 삭제한다. 로컬에 해당 CLI가 설치돼 있어야 생성이 동작한다. 러너에는 CLI가 아닌 HTTP 경로(`openai_responses`)도 있지만 `/options`의 `providers`에 없어 화면에서는 못 고른다(컨트롤러가 provider를 검증하지 않아 잡 payload로는 들어간다).
- **LLM CLI 래퍼는 두 개다 — 고칠 때 둘 다 봐라.** 글 생성·축 제안은 `llm-runner.ts`의 `runLlm()`, **학원 심층조사는 `academy-research-llm.ts`의 `runResearchCli()`**(자체 `spawnText`·파서를 따로 가진 별개 함수다). 둘 다 codex 우선이고 claude 경로에서 API 키를 지우지만, 조사 쪽은 claude로 폴백될 때만 모델을 `opus`로 명시한다. 폴백은 CLI 실패·JSON 파싱 실패에서만 일어나므로 **형식은 맞지만 채움률이 낮은 응답은 그대로 저장된다.** 한쪽만 고치면 다른 쪽은 조용히 옛 동작을 유지한다.
...
- **학원 소개·비교 글의 정본은 T16(`local_axis`)이다. T01 계열은 폐기됐다** — `constants.ts`의 `DEPRECATED_BUILTIN_TEMPLATE_IDS`에 있고, 노출 목록에 넣어도 서버(`settings/builtin-visibility`)가 걸러내 다시 켜지지 않는다. 코드(`t01-*.ts`)는 과거 발행 글·슬롯이 참조하므로 남겨 둔다. **`t01-legacy-plus.ts` 는 예외다 — 고치면 지금 글이 바뀐다.** T16 분기가 그 파일의 `legacyPlusDesignGuide`(디자인 작성 지침)·`legacyPlusArticlePatternGuide`(원본 전체 글 패턴)를 **재사용**한다(`worker.service.ts` 의 t16Plan 분기). 효과가 없는 것은 T01 **전용** 분기다 — `t01-data-gated.ts`·`t01-legacy-plus-postedit.ts` 와 Legacy Plus 컨텍스트를 받는 함수들. 지금 도는 학원 글은 T16(여러 곳 소개)·T14(단독 소개) 둘이다. 고칠 곳은 `t16-axis-comparison.ts`(T16 전용 계약·문체)와 `archetypes.ts`의 `local_axis`(구조)다. **함정: `academy-review-evidence.ts`는 이름이 중립적이지만 `selectedStudentReviewForAcademy`·`selectEligibleReviewsByAcademy`는 T01 경로에서만 불린다.** 리뷰 규칙처럼 두 계열이 공유해야 하는 판정은 연결 지점인 `academy-link.service.ts`에 두면 양쪽에 닿는다(실제로 리뷰 인용 필터가 T01 경로에만 걸려 있어 T16 글이 새던 적이 있다).
...
- **`npm run dev` 중에는 글 생성이 언제든 끊긴다.** `API_WORKER=1`이면 워커가 API 프로세스 안에서 도는데 API는 `tsx watch`라, **`apps/api-nest/src` 아래 파일이 하나라도 저장되면 재시작하며 진행 중인 생성이 죽는다.** 생성 1건은 이미지 없이 1~3분이다(발행 14건 실측: 중앙 89초·최대 148초. 이미지 생성을 켜면 더 걸린다). 죽은 잡은 `recoverStaleRunningJobs`가 `제한시간+여유(WORKER_CANCEL_EXTRA_GRACE_SEC, 기본 300초)` 초과 후 `failed`로 정리하는데, 에러 메시지가 `실패 처리됨(작업자 응답 없음...)`이라 **코드 문제와 구분되지 않는다.** 판별법: `jobs.heartbeat_at`이 `started_at`에서 거의 안 움직였으면 재시작으로 죽은 것이다. **이 판별법은 2026-07-29부터만 믿을 수 있다** — 그전에는 `heartbeat_at`이 슬롯 경계에서만 갱신돼, 살아서 LLM을 기다리는 중에도 멈춰 있었고 그 탓에 멀쩡한 잡 3건이 실패로 정리됐다(한 건은 글이 발행됐는데 잡만 실패로 남았다). 지금은 워커가 잡을 처리하는 내내 30초마다 맥을 짚으므로(`touchJobHeartbeat`), 멈춘 heartbeat는 실제로 워커가 죽었다는 뜻이다. 편집하면서 생성을 돌려야 하면 별도 셸에서 `npm run worker:once`를 쓰거나, 다른 사람이 같은 저장소를 편집 중인지 먼저 확인하라. **잡을 넣는 시점도 중요하다** — `apps/api-nest/src`를 저장한 직후에 잡을 넣으면 재시작 중인 워커가 잡을 claim 하자마자 죽어, `heartbeat_at`이 `started_at`에서 1초도 안 움직인 채 20분간 `running`으로 남는다(실측 2026-07-29).
...
- **문서도 코드를 따라 썩는다 — 다만 썩는 것은 「열거」와 「숫자」다.** 인수인계 문서(`HANDOFF.md`·`docs/admin-ui-guide.md` 등)에서 실제로 거짓이 되는 것은 라우트 목록·엔드포인트 수·탭 이름·글유형 목록 같은 **셀 수 있는 열거**이고, 설계 의도 서술은 거의 안 썩는다. 그래서 **문서에 줄번호와 개수를 적지 마라**(둘 다 이 저장소에서 실제로 밀리고 어긋났다). 값은 코드가 정본이라고 가리키고, 화면 문구는 `docs/ui-copy-inventory.md`가 정본이므로 문서로 복사하지 마라 — 복사하면 두 곳이 같이 썩는다. 열거는 `npm run verify:doc-sync`가 코드와 대조한다(생성이 아니라 **검사**다 — 표에 붙은 사람이 쓴 역할 설명을 지키기 위해서다). **화면(`app/**/page.tsx`)이나 컨트롤러 엔드포인트를 늘리고 지웠으면 이 검사를 돌려라.** 동작 서술("연결을 눌러야 반영된다")은 안내멘트 B급과 같은 이유로 기계가 판정하지 못하니 사람이 읽어야 한다.
