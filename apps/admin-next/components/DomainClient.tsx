  ...
  */
  ...
  return (
    <div>
      ...
      {view === "overview" && tab === "plan" && <><Principles domain={domainConfig} options={options} busy={busy} onSave={saveDomain} onRefresh={refresh} /><KeywordMaster domain={domainConfig.domain} presetKey={domainConfig.vertical || "driving"} keywordAxis={(payload.axes?.keyword ?? []) as AxisValue[]} onRefresh={refresh} /></>}
...
// 이미 강제되는 규칙을 읽기 전용으로 보여준다.
//
// 지금까지 이 규칙들은 코드 상수에만 있어 관리자가 볼 화면이 없었다. 그래서 "가격 안내 문구를
// 어디 적어야 하나" 처럼 **이미 적혀 있는 것**을 다시 묻게 되고, 공통 원칙 칸에 중복해서 적으면
// 강제력은 안 생기고 그 칸에서만 전달되는 말투·태도 지시만 희석된다(CLAUDE.md 경고).
function EnforcedPrinciples({ principles }: { principles?: { absolute: string; academy: string } }) {
  if (!principles?.absolute) return null;
  const lines = (text: string) => text.split("\n").map((line) => line.replace(/^-\s*/, "").trim()).filter(Boolean);
  return (
    <details className="card card-pad" style={{ background: "#f8fafc", margin: "4px 0" }}>
      <summary style={{ cursor: "pointer", fontWeight: 800 }}>
        이미 모든 글에 강제되는 규칙 <span className="badge">읽기 전용</span>
      </summary>
      <p className="muted small" style={{ margin: "8px 0" }}>
        생성 프롬프트에 항상 들어갑니다(학원 규칙은 학원 후보를 다루는 글에만). 이 중 금액 날조·전화번호
        노출·후보 수 부풀리기처럼 <b>게이트가 완성된 글을 직접 검사</b>하는 항목이 있고, 나머지는 프롬프트 지시입니다.
        어느 쪽이든 <b>아래 「공통 작성 원칙」에 다시 적지 마세요</b> — 강제력은 더 생기지 않고, 그 칸에서만
        전달되는 말투·태도 지시가 묻힙니다.
      </p>
      <b className="small">전 글 공통</b>
      <ul className="small" style={{ margin: "4px 0 12px", paddingLeft: 18 }}>
        {lines(principles.absolute).map((line, i) => <li key={i} style={{ marginBottom: 4 }}>{line}</li>)}
      </ul>
      <b className="small">학원 후보를 다루는 글(가격 안내·단서 배치 포함)</b>
...
function Principles({ domain, options, busy, onSave, onRefresh }: { domain: DomainConfig; options: AdminOptions | null; busy: boolean; onSave: (f: Record<string, unknown>) => Promise<void>; onRefresh: () => Promise<void> }) {
  ...
  return <div className="card card-pad grid" data-tour="plan-brief">
    <h2>공통 작성 원칙</h2>
    <EnforcedPrinciples principles={options?.enforced_principles} />
    ...
    <Field label="공통 작성 원칙 (모든 글 유형 공통)"><textarea className="textarea" rows={7} value={brief} onChange={(e) => setBrief(e.target.value)} placeholder="처음 준비하는 독자도 이해할 수 있는 쉬운 표현을 쓰되, 신뢰감 있는 전문가의 설명 톤을 유지한다.&#10;광고성·낚시성 문구와 근거 없는 과장 표현을 쓰지 않는다.&#10;경쟁 브랜드나 특정 업체를 비방하지 않고 균형 있게 설명한다." /><p className="muted small"><b>이 사이트만의 말투·태도</b>를 적는 칸입니다. 확인된 데이터만 사용·가격/합격률 날조 금지·후보 수 부풀리기 금지 같은 <b>안전·데이터 규칙은 이미 생성 프롬프트와 품질 게이트가 강제</b>하므로 여기에 다시 적지 않아도 됩니다. 오히려 중복해서 채우면 이 칸에서만 전달되는 말투 지시가 묻힙니다. <b>비우면 프롬프트에 「공통원칙: 없음」으로 들어갑니다</b> — 기본값이 대신 적용되지 않으니, 쓰지 않을 생각이 아니라면 비워 두지 마세요.</p></Field>
...
function CustomTemplatesManager({ domainConfig, options, keywordPool, onSave, onRefresh }: { domainConfig: DomainConfig; options: AdminOptions; keywordPool: string[]; onSave: (f: Record<string, unknown>) => Promise<void>; onRefresh: () => Promise<void> }) {
  ...
  return <section className="card card-pad grid">
    ...
    <CustomTemplateForm mode="create" domain={domain} kindOptions={kindOptions} designChoices={designChoices} sources={createSources} academyTypeOptions={academyTypeOptions} keywordPool={keywordPool} structureVariants={options.archetype_structure_variants} candidateRules={options.candidate_rules} brandColor={domainConfig.brand_color} brand={publicBrandName(domainConfig)} busy={busy}
...
function CustomTemplateForm({ mode, domain, initial, kindOptions, designChoices, sources, academyTypeOptions, keywordPool, structureVariants, candidateRules, brandColor, brand, busy, onSubmit, onClone, onCancel }: {
  ...
  sources?: TemplateSource[]; academyTypeOptions?: Array<{ value: string; count: number }>; keywordPool?: string[]; structureVariants?: Record<string, string[]>; candidateRules?: AdminOptions["candidate_rules"]; brandColor?: string | null; brand?: string; busy: boolean;
    ...
    <details className="template-subsection" open={openSec.title} onToggle={(e) => { const open = e.currentTarget.open; setOpenSec((s) => ({ ...s, title: open })); }}>
      ...
      <div className="grid" style={{ marginTop: 8, gap: 10 }}>
        ...
        <div className="info-panel small" style={{ margin: 0 }}>
          {/* 반경·상한은 서버 상수를 그대로 렌더한다. 손으로 적으면 env 로 값을 바꿨을 때 화면만 옛 숫자로 남는다. */}
          <b>&lsquo;실제 후보 수&rsquo;란?</b> 그 지역 글에 <b>소개하려고 선정된 학원 수</b>입니다 — 지역명이 맞는 <b>직접 후보</b> + {candidateRules?.nearby_km ?? 20}km 이내 <b>인근 후보</b>로 모으고(둘 다 부족하면 {candidateRules?.min_guarantee_km ?? 50}km 이내 최근접으로 <b>보장</b>), 글유형 상한(<b>비교형 최대 {candidateRules?.used_per_post ?? 5}곳 · 단독형 1곳</b>)만큼 추린 값이에요. tier의 <b>후보 수</b>·<b>min_generate</b>·<code>{"{개수}"}</code> 토큰이 모두 이 값을 가리킵니다.
...
function Academies({ domain, academies, regionAxis, busy, onSave, onRefresh }: { domain: DomainConfig; academies: Academy[]; regionAxis: AxisValue[]; busy: boolean; onSave: (f: Record<string, unknown>) => Promise<void>; onRefresh: () => Promise<void> }) {
  ...
  */
  ...
  async function unlinkAcademies() {
    if (!confirm("이 도메인에서 학원 자료를 비웁니다.\n「운전학원 자료」의 원본과 조사값은 그대로 남고, 다시 연결하면 복구됩니다. 진행할까요?")) return;
    setSyncBusy("unlink");
    setAcademyMsg("");
    try {
      const res = await api<{ deleted: number }>(`/domains/${encodeURIComponent(domain.domain)}/academies`, { method: "DELETE" });
      setAcademyMsg(`학원 ${Number(res.deleted ?? 0).toLocaleString()}곳 연결을 끊었습니다. 「학원자료 연결」로 다시 가져올 수 있습니다.`);
  ...
  return <div className="grid">
        ...
        */}
        <p className="muted small">
          ...
          {blogSyncOn === null
            ? ""
            : blogSyncOn
              ? ` — 학원 동기화가 ${ACADEMY_SYNC_DURATION_WITH_BLOG} 걸립니다. 수집만 하며 글 생성에는 쓰지 않습니다.`
    ...
    */}
    ...
    <div className="card card-pad grid">
      ...
      <p className="muted small">
        「운전학원 자료」가 원천에서 받아 둔 학원 상세(사진·별점리뷰{blogSyncOn === true ? "·블로그 리뷰" : ""} 포함)와 조사값을 이 도메인으로 가져옵니다.
        <b>원천 API 를 다시 호출하지 않아 수십 초면 끝납니다.</b> 원천 자료 자체를 새로 받으려면 「운전학원 자료」 화면에서 동기화하고 여기서 다시 연결하세요.
        지역 동기화 이후 실행을 권장하며, 위 지역 옵션은 여기에 영향을 주지 않습니다.
    ...
    <div className="card card-pad grid compact-pad" style={{ background: "#f8fafc" }}>
      ...
      <div className="grid" style={{ gap: 6, borderTop: "1px solid var(--line, #e5e7eb)", paddingTop: 10 }}>
        ...
        <p className="muted small" style={{ margin: 0 }}>
          ...
          <b>「미확인」</b> 값은 쓰이지 않습니다. 조사 대상이 아니었던 항목(원천 자료가 이미 있는 수강료·셔틀 등)도 마찬가지입니다.
          <b>합격률</b>과 원천 값을 교차검증하려고 모은 항목(학원명·주소·전화·구·동·지번)은 <b>검증완료로 올려도 글에 나가지 않습니다</b> —
          합격률은 믿을 만한 원천이 없어 조사가 긁어오는 것이 학원 자기 주장뿐이고, 나머지는 원천 값이 언제나 더 정확합니다.
        </p>
        <p className="muted small" style={{ margin: 0 }}>
        ...
        <p className="muted small" style={{ margin: 0 }}>
          바꾸면 곧바로 이 도메인에 다시 연결해 반영합니다(원천 API 를 호출하지 않아 1초 안에 끝납니다).
          조사값 자체를 새로 받으려면 「운전학원 자료」에서 조사를 돌린 뒤 <b>「학원자료 연결」</b>을 누르세요.
        </p>
      </div>
    </div>
  );
    ...
    <div className="card card-pad grid">
      ...
      */}
      ...
      <p className="muted small">
        ...
        셔틀 운행 지역은 학원자료를 가져오는 시점에 계산되므로, 사전을 새로 받은 뒤에는 2단계 「학원자료 연결」을 다시 눌러야 반영됩니다.
      </p>
    </div>
  );
}
