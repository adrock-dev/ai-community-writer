...
function Overview({ domain, counts, onTab }: { domain: DomainConfig; counts: SlotCounts; onTab: (v: string) => void }) {
  return <div className="grid">
    ...
    <div className="grid grid-2">
      ...
      <div className="card card-pad"><h2>글 유형/디자인</h2><p className="muted">글 유형 {domain.templates_enabled.length}개 · 디자인 {domain.design_template_id ?? "editorial"}</p><button className="btn" onClick={() => onTab("templates")}>디자인 고르기</button></div>
    </div>
...
function Templates({ domain, options, busy, onSave }: { domain: DomainConfig; options: AdminOptions; busy: boolean; onSave: (f: Record<string, unknown>) => Promise<void> }) {
  ...
  const [design, setDesign] = useState(domain.design_template_id ?? "editorial");
...
function DomainJobCard({ job, domain }: { job: Job; domain: DomainConfig }) {
  ...
  return <details className="card" open={job.status === "running" || job.status === "failed"}>
    ...
    <div className="card-pad grid" style={{ borderTop: "1px solid var(--line)" }}>
      ...
      <div className="writer-hint"><b>작업 옵션</b><span>엔진 {String(job.payload_obj?.provider ?? "codex")}</span><span>모델 {String(job.payload_obj?.model || "기본")}</span><span>디자인 {String(job.payload_obj?.design_template_id ?? domain.design_template_id ?? "editorial")}</span><span>웹자료 {job.payload_obj?.use_web_research === false ? "미사용" : "사용"}</span></div>
...
function Settings({ domain, options, onSave, onRefresh }: { domain: DomainConfig; options: AdminOptions; onSave: (f: Record<string, unknown>) => Promise<void>; onRefresh: () => Promise<void> }) {
  const [form, setForm] = useState({ display_name: domain.display_name, vertical: domain.vertical, theme: domain.theme, brand_color: domain.brand_color ?? "#5132d7", daily_limit: domain.daily_limit }); const [sa, setSa] = useState(""); const [url, setUrl] = useState(options.indexing.url_template);
