...
describe("공통 문체 지침", () => {
  ...
  it("문체 격식 수준을 글유형 방향성에 위임하고, 대화체는 종결어미를 넓게 허용한다", () => {
    ...
    // 모호한 양보형 필러 금지 + 구체 항목 이름 대기(전역).
    expect(p).toContain("두루뭉술한 조언·양보형 필러는 쓰지 말고");
  });
  ...
  it("기본에서도 실제 AI 상투구는 계속 금지한다", () => {
    ...
    // 이모지: 대화체는 능동(실제로 곁들여 생동감), 전문가 톤은 체크리스트 ✅ 외 제한. 어느 톤이든 목록 내 혼용만 막는다.
    expect(p).toContain("실제로 곁들여 생동감을 준다");
  ...
  it("대화체 톤에 마케팅 에너지(감탄·응원)를 허용하되 전문가 톤은 차분하게 유지한다(#3)", () => {
    const p = prompt();
    // 대화체 에너지 (의무형): 실제로 감탄·응원을 쓴다
    expect(p).toContain("느낌표와 가벼운 감탄");
    expect(p).toContain("독자를 따뜻하게 응원한다");
    // 안전 경계: 활기는 확인된 사실 위에서만, 과장·합격보장·최저가 미검증 단정 금지
