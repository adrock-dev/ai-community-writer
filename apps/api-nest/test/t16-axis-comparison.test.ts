...
describe("문체 지침", () => {
  ...
  it("여러 학원 공통 조건(수강료 단서 등)은 한 번만 밝히도록 지시한다 — 후보마다 반복 = repeated_sentence 격리 원인", () => {
    ...
    // 고지문의 자리는 마무리가 아니라 **금액 옆**이다. 한때 "시점·고지성 안내는 글 마무리 부근에 둔다"고
    // 지시했더니 마무리 섹션이 약관으로 닫혔다(통제 실험: 마무리 섹션에 고지문 4/5 → 규칙 변경 후 0/5).
    // 두 assertion 이 함께 있어야 한다 — 자리를 옮기라는 지시만 있고 마무리 반복 금지가 빠지면 양쪽에 다 나온다.
    expect(guide).toContain("금액이 적힌 자리에 모은다");
    expect(guide).toContain("마무리(상담·예약) 섹션에서는 수강료 고지를 되풀이하지 않는다");
  });
