...
export function t16WritingGuide(_slot: Row, tone: "conversational" | "expert" = "conversational"): string {
  ...
  return [
    ...
    "- 각 후보는 한 문장으로 스치지 말고 최소 3~4문장으로, 그 학원만의 개성이 드러나게 소개한다. 순서 예시: ① 다른 학원과 실제로 다른 차별점(대형·견인까지 되는 종합 과정·넓은 셔틀 운행 지역·주말/야간 운영·야간·전화상담 가능 시간·실제 도로주행 코스·자체시험·소재지 특성 등 자료로 확인된 것 중 가장 두드러진 하나)으로 연다 → ② 그 차별점이 어떤 독자에게 어떤 의미인지 이어 준다 → ③ 확인된 후기가 있으면, 그 후기가 준 인상을 담은 학원 분위기 한 문장을 카드 맨 아래 인용 바로 위에 글쓴이 말로 둔다(예: \"강사님이 하나하나 챙겨주는 분위기예요\") — 위쪽 도입이 아니라 인용 바로 위에 붙여 곧바로 인용이 근거가 되게 한다. 이 문장에도, 카드 소개 산문 어디에도 '리뷰/후기'라는 단어로 후기를 가리켜 옮기지 않고(맨 아래 인용이 출처를 밝힌다 — \"리뷰에는 ~담겨 있어요\"·\"~가 언급된 리뷰가 있는 만큼\" 식 대신 서술 금지), 원문에 없는 표현·형용사를 지어내지 않으며 인용에 실제 담긴 성격만 쓴다. 후보들이 수강료·과정·업종까지 비슷해 하드 팩트로는 차이가 거의 없는 군집이면(같은 지역 전문학원끼리 흔하다), 소재지·과정을 다시 나열해 개성인 척하지 말고 ③의 후기 인상과 ①의 미세한 실제 차이(운영·전화상담 시간, 도로주행 코스 등)에서 개성을 찾는다. 주소·전화·수강료·운영시간처럼 아래 기본 정보 불릿에 들어갈 사실은 소개 문장에서 그대로 되풀이하지 말고(불릿이 담당) 소개는 차별점·의미·후기 인상에 집중한다. 자료에 없는 정보나 '입소문·인기·유명·합격 공식' 같은 미검증 평판은 만들지 말고, 없으면 상담에서 확인할 질문으로 남긴다.",
    ...
    "- 수강료 공통 단서는 성격에 따라 나눠 배치한다: '부가세 별도·검정료 포함'처럼 금액 해석에 필요한 단서는 수강료 요약표 바로 아래(또는 수강료를 처음 제시한 곳)에 한 줄로 붙이고, '○○년 ○분기 기준'·'할인·조건·시점에 따라 달라질 수 있으니 상담으로 확인' 같은 시점·고지성 안내는 글 마무리(상담·예약 CTA) 부근에 둔다. 이 단서들을 글 도입부 리드 문장으로 올리지 않는다.",
