...
export function t16WritingGuide(slot: Row, tone: "conversational" | "expert" = "conversational"): string {
  ...
  return [
    ...
    "- ③에서 사실을 '의미'로 이을 때, 그 사실이 논리적으로 보장하지 않는 결론으로 비약하지 않는다. 예: '운영시간이 길다·늦게까지 연다 → 수업을 더 오래·늦게까지 받을 수 있다'(운영시간은 학원이 열려 있는 시간일 뿐 개인 수업 시간·수강 기간·예약 가능 시간대를 보장하지 않는다), '수강료가 낮다 → 교육 질이 낮다', '후기가 좋다 → 나도 그럴 것이다' 같은 비약 금지. 의미는 사실이 실제로 뒷받침하는 범위에서만 잇고, 불확실하면 단정하지 말고 '상담에서 확인'으로 남긴다.",
