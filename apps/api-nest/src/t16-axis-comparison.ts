...
export function t16WritingGuide(slot: Row): string {
  ...
  return [
    ...
    "- 이 글은 대화체다. 종결은 \"-습니다\"를 기본으로 \"-해요\"·\"-인데요\"·\"-거든요\"에 더해 \"-거예요\"·\"-네요\"·\"-답니다\"까지 자연스럽게 섞어 친근하게 쓰고, 같은 종결을 세 문장 이상 연속으로 쓰지 않는다. 다만 반말·유행어로 글 전체를 가볍게 만들지는 않는다.",
    "- 대화체이므로 이모지를 문장·소제목에 곁들여도 된다. 다만 감정 표현을 장식처럼 남발하지 말고, 한 목록·섹션 안에서 서로 다른 이모지를 섞지 않는다(계열 통일).",
