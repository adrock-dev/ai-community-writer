...
export function t16WritingGuide(slot: Row): string {
  ...
  return [
    ...
    "- 종결은 \"-습니다\"에 \"-해요\"·\"-인데요\"·\"-거든요\"를 자연스럽게 섞고, 같은 종결을 세 문장 이상 연속으로 쓰지 않는다. 다만 전체를 가벼운 구어체로 만들지는 않는다.",
    "- 이모지는 체크리스트 ✅처럼 같은 계열의 항목에 통일해 쓸 때만 사용한다. 문장 끝 감정 표현이나 장식으로 남발하지 않고, 한 목록·섹션 안에서 서로 다른 이모지를 섞지 않는다.",
