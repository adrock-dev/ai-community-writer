...
export function t16WritingGuide(_slot: Row, tone: "conversational" | "expert" = "conversational"): string {
  ...
  return [
    ...
    `- 도입은 검색어를 그대로 반복하지 말고, 독자가 실제로 처한 상황과 궁금증에서 연다. ${openingExample} 고를 항목이 많아 판단이 쉽지 않다는 점에 공감한 뒤 이 글에서 볼 기준과 후보로 이어 간다. 도입은 두 문단(합쳐 4~6문장)으로 충실히 쓰고, 한두 문장으로 끝내지 않는다.`,
    editorToneLine,
