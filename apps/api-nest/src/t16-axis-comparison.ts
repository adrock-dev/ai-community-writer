...
export function t16WritingGuide(_slot: Row, tone: "conversational" | "expert" = "conversational"): string {
  ...
  return [
    ...
    "- 여러 학원에 똑같이 적용되는 공통 조건(수강료의 \"부가세 별도·검정료 포함·기준 분기\" 같은 단서, 모든 후보가 같은 운영 형태라는 사실 등)은 글에서 한 번만 밝힌다. 각 학원 소개에서는 그 학원만의 값(수강료 금액, 실제 소재지, 운영 시간 등)만 쓰고 공통 단서 문장을 후보마다 되풀이하지 않는다.",
