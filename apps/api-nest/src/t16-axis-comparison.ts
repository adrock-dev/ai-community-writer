...
export function t16WritingGuide(_slot: Row, tone: "conversational" | "expert" = "conversational"): string {
  ...
  return [
    ...
    "- 각 후보는 한 문장으로 스치지 말고 최소 3~4문장으로, 그 학원만의 개성이 드러나게 소개한다. 순서 예시: ① 다른 학원과 실제로 다른 차별점(대형·견인까지 되는 종합 과정·넓은 셔틀 운행 지역·주말/야간 운영·자체시험 등 자료로 확인된 것 중 가장 두드러진 하나)으로 연다 → ② 그 차별점이 어떤 독자에게 어떤 의미인지 이어 준다 → ③ 확인된 후기가 있으면 그 후기에서 실제로 드러난 성격(친절·꼼꼼·빠른 진행 등 후기가 실제로 말한 것)을 자연스럽게 녹인다. 주소·전화·수강료·운영시간처럼 아래 기본 정보 불릿에 들어갈 사실은 소개 문장에서 그대로 되풀이하지 말고(불릿이 담당) 소개는 차별점·의미·후기 성격에 집중한다. 자료에 없는 정보나 '입소문·인기·유명·합격 공식' 같은 미검증 평판은 만들지 말고, 없으면 상담에서 확인할 질문으로 남긴다.",
