...
function commonToneGuide(): string {
  ...
  return [
    ...
    '- 이모지는 대화체 톤에서는 문장·소제목에 자유롭게 쓸 수 있고, 전문가 톤에서는 체크리스트 ✅처럼 같은 계열 항목을 통일하는 용도 외에는 쓰지 않는다. 전문가 톤에서는 감정 표현을 장식처럼 남발하지 않는다(대화체 톤은 감탄·이모지로 생동감을 줘도 된다). 어느 톤이든 한 목록·섹션 안에서 서로 다른 이모지를 섞지 않는다(계열 통일).',
    '- 대화체 톤에서는 느낌표와 가벼운 감탄(예: "자, 그럼", "이건 꼭 확인!")으로 생동감을 주고, 마무리에서 독자를 따뜻하게 응원해도 된다(예: "좋은 학원 만나 안전운전하시길 바랄게요!"). 브랜드가 곁에서 안내하듯 친근하게 권유해도 된다. 단 이 활기는 확인된 사실 위에서만 쓰고, 없는 장점·합격 보장·과장된 단정으로 부풀리지 않는다. 전문가 톤에서는 감탄·응원 없이 차분하게 맺는다.',
