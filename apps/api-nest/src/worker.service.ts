...
function reviewFactsForAcademy(row: Row, seed: string): string[] {
  ...
  if (Array.isArray(blogReviews) && blogReviews.length) {
    ...
    const links = blogReviews
      ...
      .filter(Boolean)
      .slice(0, 2);
    if (themes.length || links.length) facts.push(`긍정 블로그 리뷰글 보충자료(내부 참고, 본문 서술 안 함): ${themes.length ? themes.join(", ") : "확인"}${links.length ? ` / 참고 글 ${links.join(" | ")}` : ""}`);
  }
  return facts;
}
...
export function buildPrompt(domain: Row, slot: Row, facts: string, designTemplateId: string, archetype: Archetype | undefined, direction: string, hasAcademy: boolean, forcedTitle?: string | null, options?: GenerationPromptOptions): string {
  ...
  const academyNarrativeGuide = options?.readerFlow
    ? [
      ...
      "- 학원 카드가 시작되는 섹션은 첫 `### 학원명` 앞에 한두 문장의 도입을 둔다: 이제 어떤 학원들을 볼지 밝히고 첫 학원으로 자연스럽게 이어 간다. 대화체 톤이면 활기 있게 연다(예: \"자, 그럼 원주에서 다닐 만한 4곳을 하나씩 볼게요!\"). 실제 수강생 후기가 함께 담겼다는 점은 도입에서 짧게 알려도 되지만, 후기 내용을 도입에서 요약·해석하지 않는다(\"실제 후기에서는 ~한 반응이 보인다\" 같은 대신 서술 금지). 후기는 각 학원 카드 안 원문 인용으로 보여주고, 그 인용과 확인된 사실이 어우러져 그 학원의 분위기·개성이 자연스럽게 드러나게 한다. \"엄선·BEST·만족도 높은·추천 많은·합격률\" 같은 미검증·순위 단정은 쓰지 않는다.",
      ...
      "- 학원 소개는 각 학원에서 실제로 차이가 드러나는 면허 과정·운영 형태·자체시험·수강생 리뷰를 필요한 경우에만 활용한다. 각 학원은 반드시 `### 학원명` H3로 시작하고, 한두 문장의 자연스러운 소개 뒤에 제공된 주소·전화·수강료·셔틀 운행 지역·운영 과정 중 확인된 항목을 짧은 기본 정보 불릿으로 한 번만 정리하고, 수강생 리뷰 인용은 그 불릿 아래(카드 맨 끝)에 두되 후기가 있으면 그 인용 바로 위에 학원 분위기를 글쓴이 말로 담은 한 문장을 둔다('리뷰/후기' 단어 없이, 아래 인용이 근거)(운영 형태는 학원마다 다를 때만 불릿에 넣고, 모두 같으면 넣지 않는다). 학원마다 첫 문장과 문단 순서를 기계적으로 같게 맞추지 않는다. 주소는 기본 정보이지 추천 이유가 아니다. 정보가 부족하면 내용을 부풀리지 말고 공통 체크리스트로 한 번만 확인 행동을 안내한다.",
    ].join("\n")
    : [
...
${internalLinkGuide}
- ${options?.reviewInstruction || "본문에 인용할 수강생 리뷰는 '수강생 리뷰:' 줄로 제공된 실제 수강생 원문 1건이다. 리뷰가 있는 학원은 이 1건만 후보 설명 안에 Markdown 인용(> “원문” — 출처: 운전면허PLUS 실제 수강생 리뷰)으로 그대로 노출한다. 함께 제공되는 '수강생 반응 근거' 줄의 후기들은 그 학원 분위기를 판단하는 근거일 뿐이므로 본문에 인용하거나 옮겨 적지 않는다. 테마 요약·재서술·출처 삭제는 금지하며, 작성자·작성일·평점과 제공되지 않은 후기 문구는 쓰거나 만들지 않는다. 리뷰가 없으면 실제 후기처럼 꾸며 쓰지 말고 상담 확인 팁으로 대체한다. 후기는 그 학원의 분위기·개성을 보여주는 재료(증거)이지 분석·평가 대상이 아니다 — 원문은 인용(blockquote)으로 그대로 보여준다. 후기가 준 인상은 글쓴이가 그 학원 분위기를 자기 말로 묘사한 한 문장으로 쓸 수 있고, 그 문장은 카드 맨 아래 인용 바로 위에 둔다(바로 아래 인용이 그 근거다 — 위쪽 도입이 아니라 인용에 붙인다). 이 묘사는 인용 1건만이 아니라 그 학원에 제공된 후기 전체('수강생 리뷰' + '수강생 반응 근거')에서 공통으로 드러나는 성격을 근거로 삼는다. \"~ 분위기예요\"·\"~ 곳이에요\" 같은 단정형 직접 서술로 쓰고 \"인상이 담겨 있어요\"·\"느껴져요\"·\"전해집니다\" 같은 귀속 표현은 쓰지 않되, \"친절한 학원\"·\"좋은 곳\"처럼 학원 전체를 규정하는 평판 단정 대신 후기에 나온 장면·행동으로 묘사한다. 이때 카드 소개 산문에서는 '리뷰/후기'라는 단어로 후기를 가리켜 그 내용을 옮기지 않는다(맨 아래 인용이 이미 출처를 밝힌다) — \"실제 후기에서는 ~가 언급됐다\"·\"제공된 리뷰에는 ~담겨 있어요\"·\"~가 언급된 리뷰가 있는 만큼\" 같은 어떤 형태의 대신 서술도 금지하고, 그 단어 없이 분위기를 직접 묘사한다. \"한 건만으로 단정하기보다 상담에서 확인\" 같은 신뢰도·표본 헷지는 붙이지 않고, 원문에 없는 표현·형용사를 지어내지 않으며, 묘사는 인용에 실제 담긴 성격(친절·꼼꼼·빠른 진행 등)만 넘지 않게 쓴다. '입소문·인기·유명·합격' 같은 미검증 평판은 여전히 금지한다."}
- 긍정 블로그 리뷰글 보충자료(테마 키워드·참고 글 링크)는 내부 참고용이며 본문에 서술·요약·인용하거나 링크로 넣지 않는다 — 원문 인용이 없어 그 학원의 사실로 단정할 수 없다. 후기 증거는 학생 리뷰 원문 인용으로만 보여준다.
