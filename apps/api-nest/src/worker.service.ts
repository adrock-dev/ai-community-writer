...
${internalLinkGuide}
- ${options?.reviewInstruction || "제공된 수강생 리뷰는 슬롯별로 선택된 실제 수강생 원문 1건이다. 리뷰가 있는 학원은 이 1건만 후보 설명 안에 Markdown 인용(> “원문” — 출처: 운전면허PLUS 실제 수강생 리뷰)으로 그대로 노출한다. 테마 요약·재서술·출처 삭제는 금지하며, 작성자·작성일·평점과 제공되지 않은 후기 문구는 쓰거나 만들지 않는다. 리뷰가 없으면 실제 후기처럼 꾸며 쓰지 말고 상담 확인 팁으로 대체한다. 후기는 그 학원의 분위기·개성을 보여주는 재료(증거)이지 분석·평가 대상이 아니다 — 원문은 인용(blockquote)으로 그대로 보여준다. 후기가 준 인상은 글쓴이가 그 학원 분위기를 자기 말로 묘사하는 데 쓸 수 있다(바로 아래 인용이 그 근거다). 단 \"실제 후기에서는 ~가 언급됐다\"·\"후기에서는 ~한 경험이 남아 있다\"처럼 후기를 대신 전하는 메타서술이나 \"한 건만으로 단정하기보다 상담에서 확인\" 같은 신뢰도·표본 헷지는 붙이지 않고, 원문에 없는 표현·형용사를 지어내지 않으며, 묘사는 인용에 실제 담긴 성격(친절·꼼꼼·빠른 진행 등)만 넘지 않게 쓴다. '입소문·인기·유명·합격' 같은 미검증 평판은 여전히 금지한다."}
