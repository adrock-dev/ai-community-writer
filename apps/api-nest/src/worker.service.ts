...
export class WorkerService {
  ...
  private buildFacts(domain: string, slot: Row, opts: { maxAcademyImages?: number; perAcademyImages?: number } = {}, academyTypes?: string[], archetype?: Archetype): GenerationFacts {
    ...
    const body = academies.map((a, i) => {
      ...
      //
      // **조사값은 여기까지 와도 절반 넘게 본문에 쓰이지 않는다** — 발행 19건에 69건이 실려
      // 27건만 쓰였다(39%. 주차 17%·등록 준비물 20%·편의시설 29%). 글마다 안 쓰인 항목은
      // post-insight 의 unusedResearchLines() 가 뽑아 「이 글의 근거」 화면에 보여준다.
      //
      // **프롬프트로 올리려는 시도는 실패했다(2026-07-30).** 헤더에 「(조사)」 라벨의 뜻과 쓸
      // 자리를 설명하는 줄을 넣고 T16 슬롯 하나를 5회 생성했는데(기저 1 + 금지형 문구 1 +
      // 순수 권유형 3), 조사값 8건 중 사용은 매번 정확히 1건이었다. 문구를 넣든 빼든 바꾸든
      // 움직이지 않는다. 올리려면 프롬프트가 아니라 코드 층이어야 한다 — 글유형의 카드 계약에
      // 자리를 만들거나(허용 불릿이 계약으로 열거돼 있어 조사값이 들어갈 칸이 없다),
      // academy-distinguishing-points 의 「두드러지는 점」 계산이 조사값을 보게 하는 쪽이다.
...
- 마지막 H2 섹션은 ${brand}에서 상담·예약으로 이어지는 자연스러운 CTA로 마무리하고, 브랜드명을 3~7회 정도 자연스럽게 언급한다. **그 섹션의 소제목은 "상담"·"예약"이라는 단어로 짓지 않는다** — 발행 글 21편의 마지막 소제목이 전부 "○○에 맞춰 상담·예약하세요" 한 문장의 변주였다. 소제목은 이 글의 독자가 처한 상황이나 그가 결정해야 할 것을 가리키는 말로 짓고(섹션 안에서 상담·예약으로 안내하는 것은 그대로 한다), 다른 지역·다른 독자의 글에 그대로 옮겨 붙일 수 있는 제목이면 다시 짓는다. 비교하라고 안내할 때는 과정·비용·일정·위치·셔틀처럼 선택에 영향을 주는 항목으로 좁히고, 연락처·전화번호는 비교 대상이 아니라 연락·예약 수단으로만 쓴다(예: "연락처를 비교" 같은 표현은 쓰지 않는다).
...
export function commonToneGuide(): string {
  ...
  return [
    ...
    // 응원 문장의 예시를 지우는 이유: 예시를 주면 그대로 베낀다. "좋은 학원 만나 안전운전하시길
    // 바랄게요!"를 예시로 달아 두었더니 발행 21편 중 11편이 "좋은 학원 만나 안전운전의 첫걸음…"
    // 계열로 닫았다. 바로 아래 줄이 "지침의 예시 문구를 글자 그대로 베끼지 말라"고 하는데도 그랬다.
    // 응원하라는 요구는 남기고 문장은 글마다 짓게 한다.
    '- 대화체 톤에서는 활기차고 생동감 있게 쓴다. 도입을 밝게 열고, 느낌표와 가벼운 감탄을 실제로 쓰며, 마무리에서 독자를 따뜻하게 응원한다 — 응원 문장은 그 글의 독자·지역·상황에서 나온 말로 매번 새로 짓고, 다른 글에도 그대로 쓸 수 있는 일반적인 덕담("좋은 학원 만나 …하시길 바랄게요" 같은 틀)으로 닫지 않는다. 브랜드가 옆에서 이끌어 주듯 친근하고 설득력 있게 권유한다 — 광고처럼 밝고 적극적인 카피도 좋다. (활기는 확인된 사실 위에서만 — 없는 장점·합격 보장·최저가 같은 미검증 단정은 여전히 쓰지 않는다.) 전문가 톤에서는 감탄·응원 없이 차분하게 맺는다.',
