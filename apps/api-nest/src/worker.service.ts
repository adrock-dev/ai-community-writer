...
export function buildPrompt(domain: Row, slot: Row, facts: string, designTemplateId: string, archetype: Archetype | undefined, direction: string, hasAcademy: boolean, forcedTitle?: string | null, options?: GenerationPromptOptions): string {
  ...
  const academyNarrativeGuide = options?.readerFlow
    ? [
      ...
      "- 학원 카드가 시작되는 섹션은 첫 `### 학원명` 앞에 한두 문장의 도입을 둔다: 이제 어떤 학원들을 볼지 밝히고 첫 학원으로 자연스럽게 이어 간다. 대화체 톤이면 활기 있게 열고(예: \"자, 그럼 원주에서 다닐 만한 4곳을 하나씩 볼게요!\"), 제공된 실제 수강생 후기가 함께 담겼음을 적극적으로 알린다 — 후기에서 실제로 드러난 반응을 근거로 강조한다(예: \"실제 후기에서 강사 친절함·꼼꼼한 교육 같은 반응이 보이는 곳들이에요!\"). 단 이 강조는 제공된 후기가 실제로 말하는 범위 안에서만 하고, 후기에 없는 내용이나 \"엄선·BEST·만족도 높은·추천 많은·합격률\" 같은 미검증·순위 단정은 쓰지 않는다.",
