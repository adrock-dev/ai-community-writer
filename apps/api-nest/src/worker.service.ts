...
function commonToneGuide(): string {
  ...
  return [
    banned,
    '- 보고서가 아니라 독자에게 말을 거는 블로그 문체로 쓴다. 독자를 "여러분"으로 부르거나 "~하시죠?", "~해 보세요"처럼 직접 말을 걸어도 된다(다만 "여러분"을 문단마다 반복하지 않는다). 독자의 실제 의문을 대신하는 질문형 문장은 섹션마다 한 번까지 쓸 수 있다.',
    '- 사실을 나열한 뒤에는 그것이 독자에게 어떤 의미인지 한 문장으로 이어 준다. 종결은 "-습니다"에 "-해요"·"-인데요"·"-거든요"를 자연스럽게 섞고, 같은 종결을 세 문장 이상 연속으로 쓰지 않는다. 다만 전체를 가벼운 구어체로 만들지는 않는다.',
    '- 이모지는 체크리스트 ✅처럼 같은 계열의 항목에 통일해 쓸 때만 사용한다. 문장 끝 감정 표현이나 장식으로 이모지·감탄사를 남발하지 않고, 한 목록·섹션 안에서 서로 다른 이모지를 섞지 않는다.',
