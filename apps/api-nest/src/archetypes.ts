...
export const ARCHETYPES: Record<string, Archetype> = {
  ...
  local_single: {
    ...
    writing_guide: {
      core: [
        ...
        "그 시설의 과정·위치·운영 형태·후기 같은 확인된 자료만 쓰고, 가격·셔틀·합격률·취득 기간은 자료가 있을 때만 단정하고 없으면 상담 때 확인할 질문으로 구체화한다",
        "'며칠 완성'·'단기 취득'·'당일 합격'·'합격 보장'처럼 취득 기간이나 합격을 단정·보장하는 표현은 쓰지 않는다(기간·합격은 사람마다 달라 상담 확인 사항으로 둔다)",
      ],
      region_overlay: [
      ...
      structure_variant_labels: ["위치·동선 우선", "과정·운영 우선", "상담 체크 우선", "추천 대상 우선", "이용 흐름 우선", "FAQ 포함"],
      ...
      structure_variants: [
        ...
        // D) 추천 대상 우선(누구에게 맞나부터)
        [
          "그 시설 1곳을 단독으로 다룬다 — 비교표·BEST·'후보 N곳' 프레이밍을 만들지 않는다(요약표는 가능)",
          "이 시설이 잘 맞는 사람 유형(초보·직장인·재취득 등)을 먼저 짚는다",
          "그 근거로 시설의 과정·운영 형태·위치·동선을 확인된 자료로 전개",
          "상담·비용·일정 확인 질문으로 마무리하되, 세부 전개는 글유형 방향성을 따른다",
        ],
        // E) 이용 흐름 우선(방문→상담→등록→수강)
        [
          "그 시설 1곳을 단독으로 다룬다 — 비교표·BEST·'후보 N곳' 프레이밍을 만들지 않는다(요약표는 가능)",
          "방문·상담·등록·수강까지 이용 흐름을 먼저 그려준다",
          "그 흐름에 맞춰 시설의 과정·위치·운영 형태를 확인된 자료로 전개",
          "각 단계에서 확인할 질문(비용·일정 등)으로 마무리하되, 세부 전개는 글유형 방향성을 따른다",
        ],
        // F) FAQ 포함
        [
          "그 시설 1곳을 단독으로 다룬다 — 비교표·BEST·'후보 N곳' 프레이밍을 만들지 않는다(요약표는 가능)",
          "위치·동선과 과정·운영 형태를 확인된 자료로 소개",
          "'자주 묻는 질문'(비용·일정·준비물을 상담 때 확인하는 방법) 섹션을 둔다",
          "방문·예약 안내로 마무리하되, 세부 전개는 글유형 방향성을 따른다",
        ],
      ],
    },
  },
  ...
  local_hub: {
    ...
    writing_guide: {
      ...
      structure_variant_labels: ["생활권 우선", "후보·시험장 우선", "접수·준비 우선", "비용·시간 관점", "초보자 여정 우선", "FAQ 포함"],
      ...
      structure_variants: [
        ...
        // D) 비용·시간 관점 우선
        [
          "지역에서 비용·시간을 아끼는 관점(수강 옵션·일정·이동 시간)을 먼저 정리",
          "그 관점에서 같은 생활권의 지역 학원·시험장 후보를 표로 정리해 소개",
          "셔틀·대중교통 접근성과 접수·준비 팁을 지역 정보와 엮는다",
          "상담 전 체크리스트는 '내 출발지 기준' 질문으로 구성",
        ],
        // E) 초보자 여정 우선
        [
          "면허가 처음인 사람의 여정(학원 등록 → 필기 → 기능 → 도로주행)을 먼저 순서대로 그린다",
          "각 단계를 같은 생활권의 지역 학원·시험장 후보와 표로 엮어 소개",
          "셔틀·대중교통·생활권 동선 기준의 선택 팁을 더한다",
          "상담 전 체크리스트는 '내 출발지 기준' 질문으로 구성",
        ],
        // F) FAQ 포함
        [
          "지역 생활권과 같은 구·동의 지역 후보·시험장을 표로 정리해 소개",
          "셔틀·대중교통·선택 팁과 접수·준비 팁을 지역 정보와 엮는다",
          "'자주 묻는 질문'(지역 접수·비용·준비물을 확인하는 방법) 섹션을 둔다",
          "상담 전 체크리스트는 '내 출발지 기준' 질문으로 구성",
        ],
      ],
    },
  ...
  // T16 전용. local 과 학원 중심·지역 주축은 같지만, 섹션을 축(modifier/intent)이 채우는 점이 다르다.
  // 그래서 structure 를 고정 목록이 아니라 '슬롯이 채울 자리'로 서술한다 — 실제 열·주제·질문은
  // t16-axis-comparison.ts 가 슬롯 축 + facts 로 결정해 프롬프트에 주입한다.
  local_axis: {
    id: "local_axis", primary: "region", keyword_rule: { format: "region_plus_pick", pattern: /운전면허학원|자동차운전전문학원|자동차학원/u }, academy_centric: true,
    writing_guide: {
      core: [
        "이 글은 각 학원을 하나씩 소개·안내하는 글이다. 학원끼리 우열을 정량 비교하려 애쓰지 말고, 각 학원의 확인된 특징과 장점을 그 학원 소개 안에서 살린다",
        "확인된 후보만 소개하고 실제 후보 수보다 큰 숫자를 만들지 않는다. 각 학원의 장점은 확인된 사실에 근거할 때만 쓰고 순위·과장은 만들지 않는다",
        "후보는 이 지역에서 다닐 수 있는 범위에서 골랐다. 어느 후보가 더 가깝다거나 이동이 편하다고 단정하지 않고, 거리 수치·직선거리·소요시간은 본문에 쓰지 않는다",
        "가격·셔틀·합격률·후기는 자료가 있을 때만 단정하고, 없으면 상담 때 확인할 질문으로 구체화한다",
        "확인된 셔틀 운행 지역은 그 학원의 사실이므로 그대로 쓰되, 셔틀 자료가 없는 학원의 운행 범위를 추측하지 않는다. 셔틀은 여러 확인 근거 중 하나일 뿐이라 글 전체의 주제로 삼지 않는다",
        "대중교통 노선·도보 시간·주차 여부는 확인된 자료가 없으므로 쓰지 않고, 상담 확인 질문으로만 남긴다",
      ],
      region_overlay: [
        // 도입에만 담을 내용 명세가 없어 실행마다 흔들리고 늘 글에서 가장 얇은 섹션이 됐다(실측 4건 164~236자,
        // 다른 섹션은 295~600자). 강조·필수응답 섹션이 안정적인 이유(주제와 답할 것이 명시됨)를 도입에도 준다.
        //
        // 문단 수로 분량을 지정하는 이유: 공용 문체 지침이 "문단 하나는 2~3문장"이라 문단당 문장 수를 올리면
        // 정면 충돌한다. 또 하한을 이미 관측된 산출 범위(2문단·4~5문장) 안에 두면 모델이 그 최소값을 그대로
        // 택해 현상 추인이 된다(실측: "4~6문장"을 주자 정확히 4문장이 나왔다). 정성 표현("충실히", "얇지
        // 않게")은 무시되므로 세는 단위로만 준다.
        "도입은 세 문단으로 쓴다 — ① 슬롯 페르소나가 처한 상황과 그때 생기는 궁금증을 2~3문장으로 구체적으로 그린다 ② 그 상황에서 무엇을 먼저 확인해야 하는지 2문장 안팎으로 짚는다 ③ 이 글이 어느 범위의 후보를 다루는지 한 문장으로 밝힌 뒤 무엇을 어떤 순서로 볼지 안내한다. 세 문단을 합쳐 6~8문장이 되게 한다",
        "후보별로 '### 학원명' 소제목을 쓰고 그 학원의 확인된 특징·장점을 살려 소개한다. 실제 소재지는 사실로만 적고 추천 이유로 쓰지 않는다",
      ],
      structure: [
        "각 학원을 '### 학원명' 소제목으로 하나씩 소개하는 것이 글의 중심이다 — 확인된 특징·장점을 살리고, 카드마다 사진 1장",
        "'각 학원 소개에서 부각할 관점'이 지정한 측면을 각 학원 소개에서 살린다",
        "학원 요약표는 각 학원의 확인된 정보를 한눈에 모아 보여주는 보조 도구다 — 우열을 매기는 비교표가 아니고, 후보가 많을수록(4곳 이상) 더 충실히 둔다",
        "'강조 섹션'이 지정한 주제로 섹션을 하나 둔다",
        "'필수 응답'이 지정한 질문에 답하는 섹션을 둔다",
        "마무리에서 페르소나 상황과 학원을 확인된 사실로 연결하고 상담·예약 CTA로 닫는다",
      ],
      structure_variant_labels: ["소개 먼저", "필수 응답 먼저", "관점 강조 먼저"],
      structure_variants: [
        [
          "도입 뒤 곧바로 후보별 '### 학원명' 소개로 들어간다 — 확인된 특징·장점을 살려, 카드마다 사진 1장",
          "소개 뒤에 학원 요약표로 정보를 한눈에 정리한다(우열 비교 아님, 후보가 많을수록 충실히)",
          "'강조 섹션' 주제로 섹션을 하나 둔다",
          "'필수 응답' 질문에 답하는 섹션을 둔다",
          "페르소나 상황과 학원을 확인된 사실로 연결하는 마무리 + 상담·예약 CTA",
        ],
        [
          "도입 다음에 '필수 응답' 질문에 답하는 섹션을 먼저 둔다",
          "후보별 '### 학원명' 소개 — 확인된 특징·장점을 살려, 카드마다 사진 1장",
          "학원 요약표로 정보를 한눈에 정리한다(우열 비교 아님, 후보가 많을수록 충실히)",
          "'강조 섹션' 주제로 섹션을 하나 둔다",
          "페르소나 상황과 학원을 확인된 사실로 연결하는 마무리 + 상담·예약 CTA",
        ],
        [
          "후보별 '### 학원명' 소개로 문을 열되 '각 학원 소개에서 부각할 관점'을 앞세워 각 학원의 특징을 살린다 — 카드마다 사진 1장",
          "'강조 섹션' 주제로 섹션을 하나 둔다",
          "학원 요약표로 정보를 한눈에 정리한다(우열 비교 아님, 후보가 많을수록 충실히)",
          "'필수 응답' 질문에 답하는 섹션을 둔다",
          "페르소나 상황과 학원을 확인된 사실로 연결하는 마무리 + 상담·예약 CTA",
