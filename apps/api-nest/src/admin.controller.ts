...
export class AdminController {
  ...
  enqueueGenerate(@Req() req: Request, @Headers() headers: Record<string, string>, @Param("domain") domain: string, @Body() body: Row) {
    ...
    // 작성은 기존 planned 후보만 사용한다. 후보 생성은 「글 후보 만들기」(slots/generate) 전용이며 여기서 자동 생성하지 않는다.
    // 화면에 그대로 뜨는 메시지이므로 버튼 이름을 화면과 같게 적는다 — 「재료로 글 후보 만들기」라는
    // 버튼은 없어졌는데 메시지만 남아, 이 오류를 만난 사람을 없는 버튼으로 보내고 있었다.
    if (!slotIds.length) {
      throw new HttpException(
        "작성할 planned 후보가 없습니다. 먼저 「글 생성」 탭 1단계의 「글 후보 만들기」로 후보를 만든 뒤 작성하세요. (검색어·유형·제외 목록도 확인하세요.)",
        400,
      );
    }
