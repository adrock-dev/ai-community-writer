...
export const TEMPLATE_SPECS = {
  T01: { name: "지역 운전학원 BEST 비교", primary: ["region"], use_persona: true, modifier_count: 2, weight: 1.15, min_sv: 0, kind: "local_best", default_design: "comparison", default_direction: "지역 학원 후보를 비교표와 추천 기준으로 정리하고, 상담·비용·셔틀·면허 종류 확인으로 전환을 연결한다. 학원명·주소·전화·가격·합격률·셔틀·후기는 확인된 자료가 있을 때만 단정하고, 없으면 상담 때 확인할 항목으로 처리한다.", axis_tags: { persona: ["*"], modifier: ["*"] }, axis_values: { persona: ["운전면허학원 비교 중인 수강생", "가성비 좋은 학원을 찾는 수강생", "집 근처 학원을 찾는 수강생", "셔틀버스 이용 희망자", "야간반을 찾는 직장인", "주말만 가능한 직장인", "빠른 시험 일정이 필요한 수강생", "시험장과 가까운 학원을 찾는 수강생"], modifier: ["가까운", "셔틀편리", "비용절약", "야간반", "주말반", "상담전확인"] } },
  ...
  T07: { name: "지역 허브 총정리", primary: ["region"], use_persona: false, modifier_count: 0, weight: 1.25, min_sv: 0, with_intent: true, kind: "regional_hub", default_design: "local-guide", default_direction: "지역 단위로 학원·시험장·생활권 정보를 허브형으로 총정리해 지역 검색 의도를 폭넓게 충족한다. 학원명·주소·전화·가격·합격률·셔틀·후기는 확인된 자료가 있을 때만 단정하고, 없으면 상담 때 확인할 항목으로 처리한다.", axis_tags: { intent: ["select", "location", "common"] }, axis_values: { intent: ["근처학원", "비교추천", "비용확인", "셔틀확인", "준비물"] } },
  ...
  T11: { name: "지역 운전면허시험장 소개", primary: ["region"], use_persona: false, modifier_count: 0, weight: 1.0, min_sv: 0, with_intent: true, kind: "test_center", default_design: "local-guide", default_direction: "지역 운전면허시험장의 위치·동선·준비물·확인 포인트를 안내한다.", axis_tags: { intent: ["location", "select", "common"] }, axis_values: { intent: ["준비물", "비교추천", "비용확인", "근처학원"] } },
  T12: { name: "운전면허 취득 총정리", primary: ["keyword"], use_persona: true, modifier_count: 1, weight: 1.0, min_sv: 0, kind: "license_complete", default_design: "editorial", default_direction: "면허 취득 전 과정을 처음부터 끝까지 총정리하고, 단계별 준비물과 확인 사항을 제공한다.", axis_tags: { persona: ["license", "select", "timing", "common"], modifier: ["select", "common"] }, axis_values: { persona: ["운전이 처음인 초보자", "처음 면허 따는 대학생", "취업 전 면허 취득", "군 입대 전 면허 취득", "방학 중 단기 취득", "출퇴근을 위해 면허가 필요한 직장인"], modifier: ["필기시험부터", "비용절약", "주말반"] } },
  T13: { name: "타겟별 운전면허 준비", primary: ["keyword"], use_persona: true, modifier_count: 1, weight: 0.9, min_sv: 0, kind: "persona_target", default_design: "editorial", default_direction: "대상(페르소나)별 상황에 맞춰 면허 준비 방법과 확인 포인트를 제안한다.", axis_tags: { persona: ["*"], modifier: ["*"] }, axis_values: { persona: ["교대근무 직장인", "주말만 가능한 직장인", "대학생 방학 특강 찾는 학생", "육아를 위해 운전이 필요한 부모", "장롱면허 재도전", "외국인 운전면허 취득 준비", "취업 준비생", "자영업자 시간 맞춤 수강"], modifier: ["야간반", "주말반", "비용절약", "셔틀편리"] } },
  T14: { name: "전문학원 단독 소개", primary: ["region"], use_persona: true, modifier_count: 0, weight: 0.98, min_sv: 0, kind: "academy_profile", default_design: "conversion", default_direction: "특정 전문학원을 단독으로 소개하되 확인된 자료만 사용하고, 상담·비용·셔틀 확인으로 전환을 연결한다. 학원명·주소·전화·가격·합격률·셔틀·후기는 확인된 자료가 있을 때만 단정하고, 없으면 상담 때 확인할 항목으로 처리한다.", axis_tags: { persona: ["*"] }, axis_values: { persona: ["가성비 좋은 학원을 찾는 수강생", "집 근처 학원을 찾는 수강생", "셔틀버스 이용 희망자", "자체시험 가능한 운전전문학원을 찾는 수강생", "오늘 상담 가능한 학원을 찾는 사용자", "학원 후기 확인 중인 사용자", "시험장과 가까운 학원을 찾는 수강생"] } },
  T15: { name: "지역+시험단계 혼합", primary: ["region"], use_persona: true, modifier_count: 1, weight: 0.95, min_sv: 0, with_intent: true, kind: "local_exam_mix", default_design: "local-guide", default_direction: "지역과 시험 단계를 함께 엮어 지역 학원·시험 준비 정보를 제공한다. 학원명·주소·전화·가격·합격률·셔틀·후기는 확인된 자료가 있을 때만 단정하고, 없으면 상담 때 확인할 항목으로 처리한다.", axis_tags: { persona: ["select", "practice", "timing", "common"], intent: ["exam", "select", "common"], modifier: ["select", "practice", "common"] }, axis_values: { persona: ["운전면허학원 비교 중인 수강생", "처음 면허 따는 대학생", "시험이 걱정되는 수험생", "집 근처 학원을 찾는 수강생", "주말만 가능한 직장인"], intent: ["기능시험", "도로주행", "근처학원", "준비물"], modifier: ["가까운", "셔틀편리", "필기시험부터"] } }
} as const;
