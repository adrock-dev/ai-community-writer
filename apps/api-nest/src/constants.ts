...
- Markdown 굵게 표시는 독자가 스캔해서 '판단'하는 사실에만 아껴 쓴다: 그 글이 다루는 관점이 지목한 값(운영 시간·셔틀 운행 지역·수강료 금액·면허 과정 등)과 학원명 첫 등장이 우선이다. 부가세 별도·검정료 포함·기준 시점·상담 확인 같은 고지·단서 문구는 굵게 하지 않는다(선택 근거가 아니라 각주다). 문장 전체를 굵게 만들거나 같은 값을 반복해서 굵게 하지 않는다.
