...
// buildPrompt(worker.service)에 주입되는 「절대 원칙」 — 모든 생성에 강제되는 안전·데이터 규칙(단일 소스).
// 방향성 검증(templates/validate-direction)이 이 원문과 대조해 중복을 잡는다. 여기와 buildPrompt가 항상 같아야 한다.
export const DRIVING_ABSOLUTE_PRINCIPLES = `- API 자료는 글 재료일 뿐이다. 주 키워드/지역/제목과 직접 맞는 학원만 본문 후보·표·사진·CTA에 사용한다.
...
export const TEMPLATE_SPECS = {
  ...
  T11: { name: "지역 운전면허시험장 소개", primary: ["region"], use_persona: false, modifier_count: 0, weight: 1.0, min_sv: 0, with_intent: true, kind: "local", default_design: "local-guide", default_direction: "지역 운전면허시험장의 위치·동선·준비물·확인 포인트를 안내한다.", axis_tags: { intent: ["location", "select", "common"] }, axis_values: { intent: ["준비물", "비교추천", "비용확인", "근처학원"] }, academy_types: ["license_test_course", "license_center"], keyword_filter: ["운전면허시험장"] },
