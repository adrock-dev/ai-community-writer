...
export const TEMPLATE_SPECS = {
  ...
  T16: { name: "지역 운전학원 축 기반 비교", primary: ["region"], use_persona: true, with_intent: true, modifier_count: 2, weight: 1.2, min_sv: 0, kind: "local_axis", default_design: "comparison", default_direction: "지역을 기준으로 고른 후보를 독자 상황(페르소나)과 비교 기준(수식어)에 맞춰 정리하고, 독자가 알고 싶어 하는 질문(의도)에 반드시 답한다. 후보의 실제 소재지는 사실대로 적되 거리·이동시간 우위를 만들지 않는다.", axis_tags: { persona: ["*"], modifier: ["*"], intent: ["*"] }, axis_values: { persona: ["가성비 좋은 학원을 찾는 수강생", "가족 차량 운전을 준비하는 초보자", "결혼 전 면허를 준비하는 예비부부", "교대근무 직장인", "군 입대 전 면허 취득", "대학생 방학 특강 찾는 학생", "방학 중 단기 취득", "복학 전 면허 준비", "부모님 차량을 운전하려는 자녀", "빠른 등록이 가능한 학원을 찾는 수강생", "빠른 시험 일정이 필요한 수강생", "셔틀버스 이용 희망자", "수능 후 면허 준비", "시험장과 가까운 학원을 찾는 수강생", "신차 출고 예정자", "실제 시험 코스로 연습하고 싶은 수강생", "아이 등하교를 준비하는 부모", "야간반을 찾는 직장인", "연습을 많이 할 수 있는 학원을 찾는 수강생", "오늘 상담 가능한 학원을 찾는 사용자", "외국인 운전면허 취득 준비", "운전면허 비용을 비교하는 사용자", "운전면허학원 비교 중인 수강생", "운전이 처음인 초보자", "운전이 필요한 신입사원", "운전전문학원 추천을 찾는 사용자", "유학 출국 전 면허 준비", "육아를 위해 운전이 필요한 부모", "이직 준비 중인 직장인", "자녀 면허를 알아보는 보호자", "자영업자 시간 맞춤 수강", "자체시험 가능한 운전전문학원을 찾는 수강생", "자체시험 가능한 학원을 찾는 수강생", "장롱면허 재도전", "재수생 방학 면허 준비", "주말만 가능한 직장인", "중고차 구매 예정자", "집 가까운 학원을 찾는 수강생", "집 근처 셔틀이 있는 학원을 찾는 수강생", "차량 계약 후 면허를 준비하는 사용자", "처음 면허 따는 대학생", "첫 차 구매 예정자", "첫 출근 전 면허 준비", "출산을 앞둔 예비 부모", "출장이 많은 직장인", "출퇴근을 위해 면허가 필요한 직장인", "취업 전 면허 취득", "취업 준비생", "친구와 함께 등록하려는 대학생", "친절한 강사를 찾는 초보자", "커플이 함께 면허를 준비하는 수강생", "퇴근 후 배우는 직장인", "편입 전 면허 취득", "학원 후기 확인 중인 사용자", "형제자매와 함께 등록하는 수강생", "회사 근처 학원을 찾는 직장인", "회사 입사 예정자"], modifier: ["비용절약", "셔틀편리", "야간반", "주말반", "상담전확인", "가까운"], intent: ["과정선택", "학원유형", "비용구성", "후기확인", "일정확인"] }, academy_types: ["exam_academy", "academy"], keyword_filter: ["운전면허학원", "자동차운전전문학원", "자동차학원"] },
} as const;
