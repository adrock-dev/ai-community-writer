...
- 수강료 단서는 성격에 따라 나눠 배치하되 위치를 고정한다: '부가세 별도·검정료 포함' 같은 포함 조건 표기는 반드시 수강료 요약표 바로 아래 한 줄로만 두고 도입부·글 최상단에는 두지 않는다. 수강료 구성을 다루는 섹션이 앞쪽에 오더라도 구체적인 부가세·검정료 포함 조건 표기는 그 섹션이 아니라 표 아래 한 곳으로 모은다. '기준 시점·할인·조건·상담 확인' 같은 고지성 안내는 글 마무리 부근에 둔다.`;
