...
/**
 * 학원형 글에서 학원을 가리키는 내부 용어 '후보'(candidate)가 본문·소제목에 새어 나오는 것을
 * 독자용 표현 '학원'으로 바로잡는다. 프롬프트가 지침에서 학원을 '후보'로 지칭해 모델이 그대로 따라
 * 쓰는데(예: "수강 후보를 찾는", "후보별로 확인해 보세요"), 이는 내부 선택 프레이밍이라 독자에게 어색하다.
 * 프롬프트 지침의 '후보'도 '학원'으로 바꿔 근원을 줄이되(전 유형), 이 함수가 최종 결정적 보정을 한다.
 *
 * **학원형 글에만 적용한다**(academy_types 있는 유형). 비학원형(필기·가이드 등)은 '정답 후보'처럼
 * 정상 용례가 있을 수 있어 제외한다. facts·내부링크 후보 라벨은 본문이 아니라 영향 없다.
 */
export function normalizeAcademyTerm(markdown: string): string {
  // '후보'(모음 종성) → '학원'(자음 종성 ㄴ): 뒤따르는 조사도 종성에 맞게 바꾼다.
  // (후보를→학원을, 후보는→학원은, 후보가→학원이, 후보와→학원과, 후보로→학원으로)
  return String(markdown || "")
    .replace(/후보군/g, "학원")       // 후보군(종성 ㄴ) → 학원; 뒤 조사(을/으로 등) 그대로 정합, '학원군' 어색 방지
    .replace(/후보는/g, "학원은")
    .replace(/후보를/g, "학원을")
    .replace(/후보가/g, "학원이")
    .replace(/후보와/g, "학원과")
    .replace(/후보로/g, "학원으로")   // 후보로/후보로서 → 학원으로/학원으로서
    .replace(/후보/g, "학원");        // 나머지(후보도/후보별/후보의/단독 등 종성 무관 조사)
}
...
export function adjacentHeadingCount(markdown: string): number {
