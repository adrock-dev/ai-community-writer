...
export function t01LegacyPlusPromptContract(context: T01LegacyPlusContext): string {
  ...
  return [
    ...
    "- 슬롯의 페르소나는 이 글의 독자다. 도입에서 그 상황을 구체적으로 그리고, 확인 질문의 순서를 그 상황에 맞춰 정하며, 마무리에서 어떤 상황이면 어느 후보를 먼저 볼지 연결한다. 다만 연결의 근거는 확인된 사실(운영 과정·수강료·운영시간·셔틀 운행 지역)이어야 하고, 자료로 뒷받침되지 않는 추천 대상은 만들지 않는다.",
    ...
    "- 확인된 셔틀 운행 지역·경유지·이용 조건은 그 학원의 사실이므로 그대로 쓴다. 다만 셔틀 자료가 없는 학원의 운행 범위를 추측하거나, 주소만으로 통학 편의·접근성·가까움을 단정하지 않는다. 셔틀 운행 지역이 확인된 학원은 그 지역을 후보 소개나 비교표에 활용해도 된다.",
...
export function legacyPlusStructureGuide(context?: T01LegacyPlusContext): string {
  ...
  return [
    "도입은 이 글을 찾은 독자가 실제로 처했을 상황에서 연다. 검색어를 그대로 반복하지 말고, 페르소나가 놓인 조건(시간·비용·필요한 면허 종류·이동)을 한두 문장으로 그린 뒤, 고를 항목이 많아 판단이 쉽지 않다는 점에 짧게 공감하고, 이 글에서 어떤 기준과 후보를 볼지 안내한다. 면허 과정·주소·운영 형태 중 하나를 모든 도입의 고정 주제로 삼지 않는다.",
    ...
    "마무리에는 후보 사이의 주요 차이를 한 번 정리하고, 독자가 우선순위를 정하는 방법을 알려준 뒤, 상담 전에 확인할 내용으로 닫는다. 확인된 사실로 뒷받침되는 범위에서 \"어떤 상황이면 어느 후보를 먼저 보면 되는지\"를 한두 줄로 제시한다. 자료로 뒷받침되지 않는 추천 대상은 만들지 않는다.",
  ].join("\n");
}
...
export function legacyPlusWritingGuide(context?: T01LegacyPlusContext): string {
  ...
  return [
    ...
    "후보별 첫 문장과 문단 구조를 기계적으로 같게 맞추지 않는다. 각 후보는 '왜 이 후보가 후보에 들어왔는지 → 확인된 핵심 사실 → 그 사실이 독자에게 갖는 의미' 순으로 풀고, 과정·운영 형태·리뷰가 없는 경우에는 짧은 사실 소개만 남긴다.",
    ...
    "독자에게 말을 거는 표현을 실제로 쓴다. 예: \"어디부터 봐야 할지 고민될 수 있습니다\", \"이 학원이 왜 함께 소개되는지 궁금하실 수 있는데요\", \"운전이 처음이라면 이 부분을 먼저 확인해 보세요\", \"가격만 보고 결정하기 전에 함께 확인할 항목이 있습니다\", \"다른 지역에 있는데도 후보가 되는 이유가 있습니다\". 독자를 부를 때 \"여러분\"을 써도 되지만 문단마다 반복하지는 않는다.",
    "독자의 실제 의문을 대신하는 질문형 문장은 섹션마다 한 번까지 쓸 수 있다. 다만 모든 섹션을 질문으로 끝내지 않는다. 섹션 사이에는 앞 내용과 이어지는 전환 문장을 두되 후보마다 같은 전환 문장을 반복하지 않는다.",
...
export function legacyPlusArticlePatternGuide(context?: T01LegacyPlusContext): string {
  const plan = context ? legacyPlusComparisonPlan(context) : null;
  return `글의 뼈대는 독자의 의사결정 순서다: 지금 처한 상황 → 고를 때 확인할 기준 → ${plan?.focus === "verified_basics" ? "후보별 짧은 객관 정보" : "후보별 확인된 차이"} → 상황에 따른 선택 방법 → 상담 전 확인할 내용. 가상의 인물이나 체험을 만드는 방식이 아니라 독자가 실제로 밟는 판단 과정을 흐름으로 쓴다는 뜻이다. 표·기본 정보·체크리스트가 본문을 다시 설명하지 않게 한다. 확인된 셔틀 운행 지역은 독자가 통학 가능성을 스스로 판단할 근거이므로 적극 활용한다.`;
}

export function legacyPlusDesignGuide(): string {
  return "정보를 기계적으로 정리하는 보고서가 아니라, 독자의 고민을 이해하고 선택을 돕는 친근한 블로그 에디터의 톤을 쓴다. 친근하되 광고처럼 과장하지 않는다. 확인된 셔틀 운행 지역·경유지·이용 조건은 그 학원의 사실이므로 그대로 쓴다. 다만 셔틀 자료가 없는 학원의 운행 범위를 추측하거나, 주소만으로 통학 편의·접근성·가까움을 단정하지 않는다.";
}
