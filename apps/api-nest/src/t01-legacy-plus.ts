...
export function legacyPlusArticlePatternGuide(context?: T01LegacyPlusContext): string {
  const plan = context ? legacyPlusComparisonPlan(context) : null;
  return `독자의 면허 준비 질문에서 시작해 ${plan?.focus === "verified_basics" ? "짧은 객관 정보" : "학원별 확인된 차이와 객관 정보"}를 자연스러운 서술로 보여준 뒤 선택에 필요한 확인 순서로 마무리한다. 표·기본 정보·체크리스트가 본문을 다시 설명하지 않게 한다. 확인된 셔틀 운행 지역은 독자가 통학 가능성을 스스로 판단할 근거이므로 적극 활용한다.`;
}

export function legacyPlusDesignGuide(): string {
  return "비교·선택을 돕는 톤을 사용한다. 확인된 셔틀 운행 지역·경유지·이용 조건은 그 학원의 사실이므로 그대로 쓴다. 다만 셔틀 자료가 없는 학원의 운행 범위를 추측하거나, 주소만으로 통학 편의·접근성·가까움을 단정하지 않는다.";
}
