...
- `T01`: 지역 운전학원 BEST 비교 — **폐기됐습니다.** `DEPRECATED_BUILTIN_TEMPLATE_IDS`에 있고, 노출 목록에 넣어도 서버가 걸러냅니다. 코드(`t01-*.ts`)는 과거 발행 글·슬롯이 참조하므로 남겨 둔 것입니다. **T01 전용 분기를 고치면 효과가 없습니다** — 지금 도는 학원 글은 T16(여러 곳 소개)·T14(단독 소개)입니다. 다만 **`t01-legacy-plus.ts` 는 예외로, T16 이 그 파일의 디자인 지침·패턴 헬퍼를 재사용하므로 고치면 지금 글이 바뀝니다**(자세한 함정은 `CLAUDE.md`).
- `T11`: 지역 운전면허시험장 소개 — 살아 있습니다.
- `T14`: 전문학원 단독 소개 — 살아 있습니다.

**살아 있는 빌트인은 위 셋(`T16`·`T11`·`T14`)뿐입니다.** 나머지(`T03`~`T10`·`T12`·`T13`·`T15`)는
`T01`과 함께 2026-07-30에 폐기됐습니다 — 실전 검증을 거치지 않은 채 남아 있던 것들이고,
노출 목록에 넣어도 서버가 걸러냅니다. 코드는 과거 발행 글·슬롯이 참조하므로 남겨 뒀습니다.
되살리려면 `DEPRECATED_BUILTIN_TEMPLATE_IDS`에서 빼되, **그 유형으로 글을 몇 건 생성해
게이트·제목·분량을 확인한 뒤에 열어야 합니다.** 어느 것이 살아 있는지는 `constants.ts`가 정본입니다.
...
`academies.blog_reviews` 컬럼도 남아 있지만 블로그 리뷰는 근거로 쓰지 않습니다. **수집과 사용이 서로 다르게 막혀 있다는 점이 중요합니다** — 수집은 스위치(`runtime-config.blogReviewSyncEnabled()`, 기본 꺼짐, 「작업환경」에서 켤 수 있음)이고, 사용은 코드로 하드 오프여서 **스위치를 켜도 프롬프트에는 들어가지 않습니다**(자료만 다시 쌓입니다). 자료 자체가 오배정·중복배정이 섞여 근거로 쓸 수 없다고 판단한 결과이며, 영구 제거가 아니라 조건부 보류입니다. 판단 근거와 재개 조건은 `docs/source-field-usage.md` §3.6이 정본입니다.
