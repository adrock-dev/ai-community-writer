...
| `apps/admin-next/components/PostDetailClient.tsx:111` | muted-p | 보냈지만 본문에 나타나지 않은 값입니다. 결함이 아닙니다 — 후보가 다 같이 가진 편의시설처럼 비교 정보가 아니면 모델이 버리는 것이 맞습니다. | `apps/api-nest/src/post-insight.controller.ts#unusedResearchLines`<br>`apps/api-nest/src/academy-research-article-fields.ts` |
