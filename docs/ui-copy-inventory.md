...
| 위치 | 담체 | 안내멘트 | 종속 대상 |
| --- | --- | --- | --- |
| `components/AcademyResearchClient.tsx:122` | confirm | ${scope}, ${size}을 ${researchProvider}로 심층조사합니다(백그라운드).\n학원 1곳당 1분 안팎 걸립니다. 진행할까요? | `조사 소요시간(공유 상수 없음)` |
| `components/AcademyResearchClient.tsx:153` | confirm | 블로그리뷰를 동기화합니다. 한 곳씩 받아야 해 전체에 ${ACADEMY_SYNC_DURATION_WITH_BLOG} 걸립니다(백그라운드). 수집만 하며 글 생성에는 쓰이지 않습니다. 진행할까요?<br>_2026-07-28 수정. 소요시간을 lib/copy-facts.ts 단일 출처로 옮겼다._ | `apps/admin-next/lib/copy-facts.ts#ACADEMY_SYNC_DURATION_WITH_BLOG` |
| `components/DomainClient.tsx:395` | field:action | 이 흐름이 안정적이면 현재 검색 10개, 이후 100개로 확장하세요. | `components/DomainClient.tsx 작성 버튼 개수` |
| `components/DomainClient.tsx:682` | confirm | 키워드 마스터를 기본값(운전 프리셋 18개)으로 초기화할까요? 지금 표의 키워드·직접 추가한 값이 덮어써집니다.<br>_현재 18개로 일치. 프리셋에 항목을 더하면 세 곳(679·688·704)이 동시에 거짓이 된다._ | `apps/api-nest/src/constants.ts#PRESETS.driving.keyword` |
| `components/DomainClient.tsx:691` | muted-p | 글유형이 고르는 키워드 풀 + SEO 메트릭입니다. 월검색량·경쟁도(KD)는 슬롯 우선순위에 쓰입니다. 직접 편집하거나 「기본값으로 초기화」로 운전 프리셋 18개를 채웁니다. | `apps/api-nest/src/constants.ts#PRESETS.driving.keyword` |
...
| `components/DomainClient.tsx:63` | field:desc | 글유형 켜기 → 원천 데이터 → 후보 → 테스트 작성까지 순서대로 안내하는 생성 흐름 | — |
| `components/DomainClient.tsx:64` | field:desc | 작업 상태와 완성 글을 확인하고 export/indexing으로 넘기는 마감 흐름 | — |
| `components/DomainClient.tsx:70` | field:desc | 선택 준비(원천·공통설정·디자인) 후 글유형 켜기 → 생성 · 필요한 단계만 눌러도 됩니다 | — |
| `components/DomainClient.tsx:73` | field:desc | 공통원칙·제외어·키워드(선택) | — |
| `components/DomainClient.tsx:75` | field:desc | 만들 글 유형 선택(새 도메인 필수) | — |
| `components/DomainClient.tsx:82` | field:desc | 생성 이후 확인, 내보내기, 색인 요청 | — |
| `components/DomainClient.tsx:116` | field:body | 왜 이 정보를 찾는지 공감한 뒤, 글에서 바로 얻을 수 있는 내용을 짧게 알려줍니다. | — |
| `components/DomainClient.tsx:117` | field:body | 절차, 비용, 기간을 순서대로 풀고 중간에 이미지를 배치합니다. | — |
| `components/DomainClient.tsx:118` | field:body | 처음 등록해도 되나요?\|주말에도 가능한가요?\|추가 비용은 언제 생기나요? | — |
| `components/DomainClient.tsx:119` | field:body | 주변 학원 찾기나 예약 확인으로 부드럽게 연결합니다. | — |
| `components/DomainClient.tsx:130` | field:body | 가격, 셔틀, 주말 수업, 도로주행 코스를 같은 기준으로 맞춰 비교합니다. | — |
| `components/DomainClient.tsx:131` | field:body | 표 아래에는 왜 이 항목이 중요한지 짧게 해석하는 문단이 붙습니다. | — |
| `components/DomainClient.tsx:132` | field:body | 직장인, 대학생, 장롱면허처럼 상황별 추천을 분리합니다. | — |
| `components/DomainClient.tsx:133` | field:body | 가까운 학원과 예약 가능한 시간을 확인하도록 연결합니다. | — |
| `components/DomainClient.tsx:144` | field:body | 송파, 잠실, 문정처럼 생활권이 다른 사용자의 이동 동선을 나눠 설명합니다. | — |
| `components/DomainClient.tsx:145` | field:body | 집/학교와 가까운지\|셔틀 시간이 맞는지\|도로주행 코스가 어렵지 않은지 | — |
| `components/DomainClient.tsx:146` | field:body | 퇴근 후 수업을 잡을 수 있어서 주말에 몰아서 배우는 부담이 줄었다는 식의 현실적인 후기를 넣습니다. | — |
| `components/DomainClient.tsx:147` | field:body | 내 위치 기준으로 가까운 학원을 찾도록 연결합니다. | — |
| `components/DomainClient.tsx:158` | field:body | 신분증, 시험 시간, 코스 확인처럼 놓치면 바로 문제가 되는 항목을 맨 위에 둡니다. | — |
| `components/DomainClient.tsx:159` | field:body | 신분증 챙기기\|시험장 도착 시간 확인\|좌석/거울 조정 연습\|감점 포인트 복습 | — |
| `components/DomainClient.tsx:160` | field:body | 방향지시등, 일시정지, 속도 조절처럼 반복되는 실수를 실제 상황 중심으로 설명합니다. | — |
| `components/DomainClient.tsx:161` | field:body | 불안한 구간만 추가 연습할 수 있는 학원/강습 탐색으로 이어집니다. | — |
| `components/DomainClient.tsx:172` | field:body | 시간과 비용이 동시에 부담되는 상황을 구체적으로 짚어 이탈을 줄입니다. | — |
| `components/DomainClient.tsx:173` | field:body | 단기반, 셔틀, 추가 비용 여부를 상담 전 질문 목록으로 정리합니다. | — |
| `components/DomainClient.tsx:174` | field:body | 상담 후 전체 일정을 한 번에 잡을 수 있어 편했다는 톤으로 신뢰를 보강합니다. | — |
| `components/DomainClient.tsx:175` | field:body | 비용과 가능한 일정을 바로 확인하는 버튼을 강하게 보여줍니다. | — |
| `components/DomainClient.tsx:186` | field:body | 제목, 핵심 요약, 대표 이미지 등 직접 적은 규칙을 발행 렌더러가 참고할 수 있게 저장합니다. | — |
| `components/DomainClient.tsx:187` | field:body | 표, 이미지, CTA 위치처럼 반복될 디자인 규칙을 명시합니다. | — |
...
| `components/DomainClient.tsx:392` | field:action | 버튼을 누르면 작업 큐 탭에서 진행 상태를 확인합니다. | — |
| `components/DomainClient.tsx:394` | field:body | 큐에 등록된 글 생성 작업이 대기·진행·완료·실패 중 어디에 있는지 봅니다. 실패하면 상세 카드의 에러를 확인하고 같은 조건으로 다시 시도합니다. | — |
| `components/DomainClient.tsx:394` | field:action | 완료 후 검수·내보내기 탭에서 결과를 검수합니다. | — |
| `components/DomainClient.tsx:475` | muted-p | 현재 단계의 대상 영역을 찾는 중입니다. 탭을 전환했거나 데이터가 아직 로딩 중이면 잠시 뒤 다시 표시됩니다. | — |
| `components/DomainClient.tsx:535` | muted-p | 「글 생성」 흐름은 원천 데이터·공통 설정·디자인(모두 선택) 준비 후 글유형 켜기(필수)로 이어지고, 후보 만들기·테스트 작성으로 마무리합니다. | — |
| `components/DomainClient.tsx:550` | muted-p | 「글 생성 흐름 시작」을 누르면 위 순서대로 카드 영역을 포커싱합니다. | — |
| `components/DomainClient.tsx:566` | jsx-text | 큰 흐름 안에서도 필요한 작업만 바로 열 수 있습니다 | — |
...
| `components/DomainClient.tsx:604` | field:desc | ${counts.published.toLocaleString()}개 완성 글이 있습니다. 미리보기 후 Markdown/HTML export와 색인 요청으로 마감하세요. | — |
...
| `components/DomainClient.tsx:764` | muted-p | 이 도메인에서 쓸 글 유형을 켜고 끕니다(빌트인·커스텀 함께, 즉시 저장). 커스텀 유형은 맨 아래에서 만들고 여기서 켜세요. | — |
| `components/DomainClient.tsx:766` | muted-p | 담긴 글 유형이 없습니다. 아래 카탈로그에서 필요한 유형을 추가하세요. | — |
...
| `components/DomainClient.tsx:912` | confirm | 커스텀 글유형 '${t.name}'을 삭제할까요? 이미 생성된 글에는 영향이 없습니다. | — |
| `components/DomainClient.tsx:958` | muted-p | 이 글유형은 학원 근거형이 아니거나 학원 타입이 선택되지 않아 커버리지 정보가 없습니다. | — |
| `components/DomainClient.tsx:960` | muted-p | 학원 타입: {data.academy_types.join(", ")} · 충분 기준 {data.threshold}곳 이상. 상태: 충분(직접+인근 {data.nearby_km}km로 {data.threshold}곳) {data.regions_with_min_for_best} · 보장({data.min_guarantee_k… | — |
| `components/DomainClient.tsx:1111` | confirm | persona 사용을 켰으면 값을 한 줄 이상 입력하세요. (비우려면 사용을 끄세요) | — |
| `components/DomainClient.tsx:1112` | confirm | intent 사용을 켰으면 값을 한 줄 이상 입력하세요. (비우려면 사용을 끄세요) | — |
| `components/DomainClient.tsx:1113` | confirm | modifier 사용을 켰으면 값을 한 줄 이상 입력하세요. (비우려면 사용을 끄세요) | — |
| `components/DomainClient.tsx:1136` | confirm | 참조 아키타입을 선택하세요. | — |
| `components/DomainClient.tsx:1141` | info-div | {mode === "create" ? "새 커스텀 글유형" : `편집 · ${initial?.template_id}`}{mode === "edit" && 편집 중} | — |
...
| `components/DomainClient.tsx:1243` | muted-p | 먼저 학원 동기화를 실행하면 타입 목록이 표시됩니다. | — |
| `components/DomainClient.tsx:1245` | muted-p | 아직 이 도메인에 동기화된 학원이 없습니다(모든 타입 0건). 학원 타입을 골라도 실제 후보가 없어 지역 가이드/체크리스트로만 작성됩니다 — 먼저 「원천 데이터」 탭에서 학원 동기화를 실행하세요. | — |
| `components/DomainClient.tsx:1256` | muted-p | 위에서 고른 디자인의 레이아웃만 보여주는 예시 목업입니다. 실제 글 내용·방향성·축 값은 반영하지 않습니다. | — |
| `components/DomainClient.tsx:1265` | muted-p | 생성 시점 실제 후보 수로 제목 확정 · 미설정이면 LLM이 H1 결정 | — |
