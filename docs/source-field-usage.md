...
**결정된 것**: 글 생성 사용을 전역 스위치로 켜지 않는다. 전역 스위치는 맞는 90% 와 틀린 10% 를
구분하지 못해서, 켜는 순간 위 문제가 그대로 돌아온다. 수집 토글(설정 화면)은 이름 그대로
수집만 켜고 끈다.

**아직 정하지 않은 것**: 그러면 무엇을 관문으로 둘 것인가. 지금 유력한 방향은 블로그리뷰
레코드마다 검증 상태를 두고 검증된 것만 생성에 흘리는 것이다. 검증된 게 0건이면 아무것도 안
들어가므로 자동으로 안전하고, 조사값이 `status` 를 관문으로 두기로 한 것과 같은 방식이라
관문 개념이 둘로 갈라지지 않는다. 다만 **검증을 무엇으로 어떻게 할지가 먼저이고, 그 모양에 따라
관문 설계도 달라질 수 있다.** 여기서 미리 못박지 않는다.
