...
> ⚠️ **상태(2026-07-13)**: 2a→P4→P5 완료 + 축 값 프리셋(Stage1/2b/2a)·디자인/글유형 탭 대정비·글유형 on/off 통합 완료. 아래 "PR3 = design_template_overrides → template_overrides 통합" 계획은 이후 **③ 제거 + clone→커스텀** 방향으로 재정리됨(디자인은 커스텀 글유형이 소유). 진행 상세는 memory `archetype-refactor-roadmap`.
